...
export const NewDocument = () => {
    ...
    const createNewDoc = async (title: string = "Untitled Document", description: string = "") => {
        try {
    ...
    const TemplateMap = [
            ...
            component: (
                <button onClick={() => createNewDoc()}>
                    <Card className='w-[150px] hover:border-blue-500 hover:cursor-pointer'>
                        ...
                        <CardContent>
                            <Plus size={80} className='flex justify-center mx-auto' />
                        </CardContent>
        ...
        {
            component: (
                <button onClick={() => createNewDoc("Wizard template",
                    `
                [Exposition]
        ...
        {
            component: (
                <button onClick={() => createNewDoc("Resume template",
                    `
                    [Name]
                ...
                <div className="flex space-x-4 flex-wrap">
                    {
                        TemplateMap.map((template) => (
                            <div key={template.footer}>
                                {template.component}
                                <p className="text-sm mt-2 ml2">{template.footer}</p>
                            </div >
                        ))
                    }
                </div>
